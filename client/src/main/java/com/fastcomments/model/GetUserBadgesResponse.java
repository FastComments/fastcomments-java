...
public class GetUserBadgesResponse extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(GetUserBadgesResponse.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!GetUserBadgesResponse.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'GetUserBadgesResponse' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<GetUserBadgesResponse>() {
                @Override
                public void write(JsonWriter out, GetUserBadgesResponse value) throws IOException {
                ...
                @Override
                public GetUserBadgesResponse read(JsonReader in) throws IOException {
                    ...
                    try {
                        ...
                        GetUserBadgesResponse ret = new GetUserBadgesResponse();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    } catch (Exception e) {
                        // deserialization failed, continue
                        errorMessages.add(String.format(java.util.Locale.ROOT, "Deserialization for APIGetUserBadgesResponse failed with `%s`.", e.getMessage()));
                    ...
                    throw new IOException(String.format(java.util.Locale.ROOT, "Failed deserialization for GetUserBadgesResponse: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public GetUserBadgesResponse() {
        super("anyOf", Boolean.FALSE);
    }

    public GetUserBadgesResponse(Object o) {
        super("anyOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return GetUserBadgesResponse.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to GetUserBadgesResponse
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format(java.util.Locale.ROOT, "The JSON string is invalid for GetUserBadgesResponse with anyOf schemas: APIError, APIGetUserBadgesResponse. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }

    /**
     * Create an instance of GetUserBadgesResponse given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of GetUserBadgesResponse
     * @throws IOException if the JSON string is invalid with respect to GetUserBadgesResponse
     */
    public static GetUserBadgesResponse fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, GetUserBadgesResponse.class);
    }

    /**
     * Convert an instance of GetUserBadgesResponse to an JSON string
     *
     * @return JSON string
     */
